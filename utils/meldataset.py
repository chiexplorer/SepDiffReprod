...
def mel_spectrogram(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False):
    ...
    if fmax not in mel_basis:
        ...
        hann_window[str(y.device)] = torch.hann_window(win_size).to(y.device)  # hann窗
    # 以反射模式填充语音信号，去除信号边缘可能被窗口截断带来的影响
...
if __name__ == '__main__':
    # 对比~的mel spec和librosa的mel spec区别
    N_FFT = 1024
    NUM_MELS = 80
    SAMPLERATE =  22050
    HOP_SIZE = 256
    ...
    FMIN = 0
    FMAX = 8000
    fpath = r"G:\Resource\Datasets\Aishell1Mix2\wav8k\min\test\mix_clean\BAC009S0764W0122_BAC009S0767W0126.wav"
    y, sr = sf.read(fpath)
    print("采样率: ", sr)
    wav = torch.FloatTensor(y)
    mel_spec = mel_spectrogram(wav.unsqueeze(0), n_fft=N_FFT, num_mels=NUM_MELS, sampling_rate=SAMPLERATE,
                               hop_size=HOP_SIZE, win_size=WIN_SIZE, fmin=FMIN, fmax=FMAX)
    mel_spec_librosa = librosa.feature.melspectrogram(y=y, sr=SAMPLERATE, n_fft=N_FFT, hop_length=HOP_SIZE, win_length=WIN_SIZE, n_mels=NUM_MELS, fmin=FMIN, fmax=FMAX)